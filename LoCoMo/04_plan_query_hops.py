...
Plan retrieval with the narrow, specific predicate that the question asks for.
Keep research, picnic, camping, painting, travel, attend, and support distinct
when the wording identifies them. Do not broaden a specific action to activity.
""".strip(),
    "broad": """
PROPERTY-GRANULARITY CONDITION: BROAD
Plan retrieval using the same intermediate property level as the broad index.
Explicit action-like activities such as research, picnic, camping, painting,
hiking, swimming, cooking, and travel should use activity when appropriate,
while their concrete details remain in value. Keep support, preference, emotion,
identity, goal, diagnosis, time, location, pet_name, and relationship distinct.
""".strip(),
}
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=("fine", "broad"),
        default="fine",
        help="Query-planning property granularity; must match Step 3",
    )
    parser.add_argument(
        "--force-replan",
        action="store_true",
    ...
    schema = (
        "entity-structured-v2.7-prompt-driven-audited-query-plan-with-retrieval-"
        f"scope-gpt41-exhaustive-efficiency-v3-{args.property_granularity}"
    )
    ...
    if checkpoint.get("property_granularity") not in {None, args.property_granularity}:
        raise ValueError(
            "Existing query-plan checkpoint uses a different property granularity; "
    ...
    checkpoint["property_granularity"] = args.property_granularity
    ...
    for position, question in pending:
        ...
        ) + "\n\n" + PROPERTY_GRANULARITY_GUIDANCE[args.property_granularity]
