...
def main() -> None:
    ...
    parser.add_argument(
        "--fixed-top-k-ablation",
        action="store_true",
        help=(
            "Ablation mode: force every hop, including all_matching/aggregation "
            "plans, to select at most --top-k-per-hop episodes. Default retrieval "
            "behavior is unchanged when this flag is omitted."
        ),
    )
    ...
    for query_position, item in enumerate(plan_source["plans"], 1):
        ...
        for hop in plan["hops"]:
            ...
            effective_retrieval_scope = (
                "fixed_top_k" if args.fixed_top_k_ablation else retrieval_scope
            )
            if args.fixed_top_k_ablation:
                selected_rankings = ranked_episodes[: args.top_k_per_hop]
            elif retrieval_scope == "all_matching":
            ...
            if args.fixed_top_k_ablation:
                # Keep the budget fixed after structured and fallback
                # candidates are combined.  In particular, an all_matching
                # plan must not silently expand to the full episode store.
                selected_ids = selected_ids[: args.top_k_per_hop]
            ...
            hop_results.append(
                {
                    **hop,
                    ...
                    "retrieval_scope": effective_retrieval_scope,
                    "bridge_value": bridge,
        ...
        retrievals.append(
            {
                ...
                "retrieval_scope": (
                    "fixed_top_k"
                    if args.fixed_top_k_ablation
                    else plan.get("retrieval_scope", "point")
                ),
                "planned_retrieval_scope": plan.get("retrieval_scope", "point"),
                "fixed_top_k_ablation": args.fixed_top_k_ablation,
                ...
                "maximum_episode_budget_before_deduplication": (
                    len(plan["hops"]) * args.top_k_per_hop
                    if args.fixed_top_k_ablation
                    else (
                        None
    ...
    write_json(
        ...
        {
            "schema_version": (
                "entity-structured-v2.7-prompt-driven-chunked-dense-recall-theme-episode-"
                "retrieval-gpt41-fixed-top-k-ablation-efficiency-v1"
                if args.fixed_top_k_ablation
                else "entity-structured-v2.7-prompt-driven-chunked-dense-recall-theme-episode-retrieval-gpt41-exhaustive-efficiency-v3"
            ),
            ...
            "ranking_policy": {
                ...
                "budget_policy": (
                    "fixed top-k per hop for every query type"
                    if args.fixed_top_k_ablation
                    else "query-type-aware retrieval scope"
                ),
                "routing_gates": {
