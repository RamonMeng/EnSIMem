...
PROPERTY_GRANULARITY_GUIDANCE = {
    "fine": """
PROPERTY-GRANULARITY CONDITION: FINE
Use an almost surface-faithful but still grammatical semantic predicate. Preserve
the specific action or relation that distinguishes the fact: research, picnic,
camping, painting, travel, attend, and support must remain distinct properties
when explicitly stated. Do not replace a specific action with activity. Keep the
specific object/event/location in value. The property must still be a reusable
predicate, not a full sentence or a subject-prefixed label.
""".strip(),
    "broad": """
PROPERTY-GRANULARITY CONDITION: BROAD
Use a stable intermediate predicate vocabulary. Explicit action-like activities
such as research, picnic, camping, painting, hiking, swimming, cooking, and
travel may share property=activity, while their concrete details remain in
value. Do not collapse semantically different non-activity relations: support,
preference, emotion, identity, goal, diagnosis, time, location, pet_name, and
relationship must retain their own properties. Broad does not mean replacing
every fact with activity; only use activity for an explicitly stated action.
""".strip(),
}


def extract_episode(client, episode, property_granularity: str):
    ...
    response = client.chat_json(
        ...
        + "\n\n"
        + PROPERTY_GRANULARITY_GUIDANCE[property_granularity],
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=("fine", "broad"),
        default="fine",
        help="Extraction prompt granularity for the ablation",
    )
    parser.add_argument(
        "--force-rebuild",
        action="store_true",
    ...
    schema = (
        "entity-structured-v2.6-prompt-driven-semantic-atomic-index-gpt41-"
        f"exhaustive-image-aware-{args.property_granularity}"
    )
    checkpoint = read_json(destination) if destination.exists() and not args.force_rebuild else {
        ...
        "property_granularity": args.property_granularity,
        "property_policy": (
            "surface-faithful specific predicates; activity only when unspecified"
            if args.property_granularity == "fine"
            else "intermediate action predicates; explicit action variants may share activity"
        ),
    ...
    if (
        ...
        or checkpoint.get("property_granularity") != args.property_granularity
    ):
        raise ValueError(
    ...
    for offset in range(0, len(pending), args.batch_size):
        ...
        with ThreadPoolExecutor(max_workers=len(batch)) as executor:
            futures = {
                executor.submit(extract_episode, client, episode, args.property_granularity): episode
