...
class VLLMClient:
    ...
    def chat_json(self, system: str, user: str, *, max_tokens: int = 2048) -> dict[str, Any]:
        ...
        try:
            ...
            repair_system = (
                "Repair the supplied model output into exactly one valid JSON object. Output JSON only. "
                "The source may be truncated: preserve complete fields, omit incomplete optional data, "
                "and keep strings/quotes short."
            )
            try:
                repaired = self.chat(repair_system, first, max_tokens=max_tokens)
                return extract_json_object(repaired)
            except LLMError:
                # A truncated repair should not terminate a long run.  Ask once more for a compact
                # object; downstream validation will mark omitted evidence as missing rather than
                # treating malformed text as a successful extraction.
                compact = self.chat(
                    repair_system + " Return a compact object with the same top-level schema.",
                    first[:24000],
                    max_tokens=min(6000, max(2048, max_tokens * 2)),
                )
                return extract_json_object(compact)
...
class OpenAIClient(VLLMClient):
    ...
    def chat_json(self, system: str, user: str, *, max_tokens: int = 2048) -> dict[str, Any]:
        ...
        try:
            ...
            # A repair request is retained for transient/truncated responses.  If that repair is
            # itself truncated, request one compact object so a single oversized audit cannot
            ...
            try:
                repaired = self.chat(repair_system, content, max_tokens=max_tokens)
                return extract_json_object(repaired)
            except LLMError:
                compact = self.chat(
                    repair_system + " Return a compact object with the same top-level schema.",
